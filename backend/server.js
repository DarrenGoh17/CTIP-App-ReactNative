...
app.post('/api/user-preference/:userId', (req, res) => {
    ...
    // SQL query to fetch the current count for the specified type
    const fetchSql = `SELECT ${type}_count FROM user_preference WHERE user_id = ?`;

    db.query(fetchSql, [userId], (err, result) => {
        if (err) {
            return res.status(500).send({ error: 'Database error during fetch' });
        }
        ...
        const currentCount = result[0][`${type}_count`];
        console.log(`${type} count before update:`, currentCount);
        ...
        db.query(updateSql, [userId], (err, updateResult) => {
            ...
            // Fetch the updated count after the update
            db.query(fetchSql, [userId], (err, resultAfterUpdate) => {
                if (err) {
                    return res.status(500).send({ error: 'Database error during fetch after update' });
                }

                const updatedCount = resultAfterUpdate[0][`${type}_count`];
                console.log(`${type} count after update:`, updatedCount);
...
app.post('/api/request-otp', (req, res) => {
    ...
    db.query(deleteOtpSql, [email], (deleteErr) => {
        if (deleteErr) {
            return res.status(500).send({ error: 'Database error while clearing previous OTP' });
        }
        ...
        const insertOtpSql = `
            ...
            VALUES (?, ?)
        `;
        db.query(insertOtpSql, [email, otpCode], (insertErr) => {
            if (insertErr) {
                return res.status(500).send({ error: 'Database error while storing OTP' });
            }
